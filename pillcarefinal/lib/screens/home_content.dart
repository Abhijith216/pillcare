  ...
  Widget _statCard(String title, String value, String unit, IconData icon, Color iconBg, Color iconColor, Color decorBg) {
    ...
    return Container(
      ...
      child: Stack(children: [
        Positioned(right: -32, top: -32, child: Container(width: 96, height: 96, decoration: BoxDecoration(color: isDark ? iconBg.withValues(alpha: 0.2) : decorBg, shape: BoxShape.circle))),
        Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Container(width: 32, height: 32, decoration: BoxDecoration(color: isDark ? iconBg.withValues(alpha: 0.3) : iconBg, shape: BoxShape.circle), child: Icon(icon, color: iconColor, size: 16)),
          ...
          RichText(text: TextSpan(children: [
            TextSpan(text: '$value ', style: GoogleFonts.manrope(fontSize: 24, fontWeight: FontWeight.w800, color: isDark ? const Color(0xFFE2E8F0) : const Color(0xFF0F172A))),
...
class _MedicationsSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ListenableBuilder(
      ...
      builder: (context, _) {
        final store = MedicationStore();
        // Only show caregiver-approved medications on the home screen
        final meds = store.approvedMedications;
        final pendingCount = store.pendingMedications.length;
        final theme = Theme.of(context);
        final isDark = theme.brightness == Brightness.dark;
        ...
        return Column(children: [
          Padding(
            ...
            child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Text("Today's Medications", style: GoogleFonts.manrope(fontSize: 20, fontWeight: FontWeight.w800, color: isDark ? const Color(0xFFE2E8F0) : const Color(0xFF1E293B))),
              ElevatedButton(
                onPressed: () async {
            ...
            Padding(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: const Color(0xFFFDE047).withValues(alpha: 0.7)),
                ),
                child: Row(
                  children: [
                    const Icon(Icons.hourglass_top_rounded, size: 18, color: Color(0xFFCA8A04)),
                    const SizedBox(width: 8),
                    Expanded(
                      child: Text(
                        '$pendingCount medication${pendingCount > 1 ? 's' : ''} awaiting caregiver approval',
                        style: GoogleFonts.manrope(
                          fontSize: 12,
            ...
            child: meds.isEmpty
              ? Container(
                  ...
                  child: Column(children: [
                    Icon(Icons.medication_outlined, size: 48, color: theme.colorScheme.onSurface.withValues(alpha: 0.3)),
                    const SizedBox(height: 12),
                    Text('No approved medications', style: GoogleFonts.manrope(fontSize: 16, fontWeight: FontWeight.w700, color: theme.colorScheme.onSurface.withValues(alpha: 0.5))),
                    const SizedBox(height: 4),
                    Text(pendingCount > 0 ? 'Waiting for caregiver to approve' : 'Tap + to add your first medication', style: GoogleFonts.manrope(fontSize: 13, color: theme.colorScheme.onSurface.withValues(alpha: 0.3))),
                  ]),
                )
              : Column(
      ...
      child: Column(children: [
        Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(m.name, style: GoogleFonts.manrope(fontSize: 17, fontWeight: FontWeight.w700, color: textPrimary)),
            const SizedBox(height: 2),
            Text(m.displayDosage, style: GoogleFonts.manrope(fontSize: 13, color: textSecondary)),
          ])),
          IconButton(
            onPressed: () async {
              final result = await showModalBottomSheet<Medication>(
            ...
            icon: Icon(Icons.edit, size: 20, color: textSecondary),
          ),
        ]),
        const SizedBox(height: 12),
        ...
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Row(children: [
            ...
            const SizedBox(width: 8),
            Text(m.displayTime, style: GoogleFonts.manrope(fontSize: 14, fontWeight: FontWeight.w600, color: isDark ? const Color(0xFFCBD5E1) : const Color(0xFF475569))),
          ]),
          _btn(btnStyle, m.id),
        ]),
        // Refill indicator
